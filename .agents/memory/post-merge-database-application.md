...
**How to apply:** After schema-bearing merges, require either a successful Drizzle migration or an exact latest-snapshot catalog match. Any drift must fail setup. Keep production release checks blocked until operator-confirmed ledger adoption is complete; never use push, forced migration, or synthetic ledger rows as a fallback.