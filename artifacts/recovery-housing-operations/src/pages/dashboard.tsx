  ...
  return <AppShell><div className="animate-enter">
    ...
    <QueryState loading={dashboard.isLoading} error={dashboard.isError} retry={() => queryClient.invalidateQueries({ queryKey: getGetDashboardQueryKey() })}>
      {d && <div className="mt-9 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Metric icon={<UsersRound size={18} />} label="Active residents" value={d.activeResidents} note={`${counts.pending ?? 0} preparing to move in`} tone="teal" />
        ...
        <Metric icon={<ClipboardList size={18} />} label="Payments due" value={d.paymentsDue} note="Across current residents" tone="coral" />
        <Metric icon={<CircleDollarSign size={18} />} label="Collected this month" value={money(d.paymentsCollected)} note="On-time and recorded" tone="ink" />
      </div>}
    </QueryState>
...
function Metric({ icon, label, value, note, tone }: { icon: React.ReactNode; label: string; value: string | number; note: string; tone: string }) {
  const tones: Record<string, string> = { teal: 'bg-[hsl(169_32%_87%)] text-[hsl(var(--primary))]', sand: 'bg-[hsl(38_66%_88%)] text-[hsl(31_70%_34%)]', coral: 'bg-[hsl(14_72%_88%)] text-[hsl(14_58%_35%)]', ink: 'bg-[hsl(195_26%_84%)] text-[hsl(195_32%_23%)]' };
