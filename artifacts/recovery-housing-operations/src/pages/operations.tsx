...
import { canImportResidents, canViewReports, confirmResidentImport, downloadResidentTemplate, exportReport, get, getReportPreview, isAdministrator, previewResidentImport, reportFilename, reportTypes, type ReportFormat, type ReportType } from '@/lib/api';
...
export default function Operations() {
  ...
  return <AppShell><div className="animate-enter">
    ...
    <QueryState loading={active?.isLoading} error={active?.isError} retry={() => active?.refetch()}><div className="paper-card mt-6 overflow-hidden">
      ...
      {section === 'import' && <ResidentImport />}
      {section === 'reports' && <Reports data={reports.data} houses={houses.data ?? []} />}
    </div></QueryState>
...
function Reports({ data, houses }: { data?: any; houses: any[] }) {
...
function LegacyReports({ data }: { data?: any }) {
  const [reportType, setReportType] = useState<ReportType>('occupancy');
  const [format, setFormat] = useState<ReportFormat>('csv');
