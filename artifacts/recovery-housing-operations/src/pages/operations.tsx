import { ClipboardCheck, Download, FileText, Home, Plus, RefreshCw, ShieldCheck, Upload, UsersRound } from 'lucide-react';
...
export default function Operations() {
  ...
  const [section, setSection] = useState<'applications' | 'houses' | 'daily' | 'documents' | 'reports'>('applications');
  ...
  const active = section === 'applications' ? applications : section === 'houses' ? houses : section === 'daily' ? daily : section === 'documents' ? documents : reports;
  ...
  return <AppShell><div className="animate-enter">
    ...
    <div className="mt-8 grid gap-3 sm:grid-cols-4">{[
      ['applications', 'Applications', ClipboardCheck], ['houses', 'Houses & beds', Home], ['daily', 'Daily operations', ShieldCheck], ['documents', 'Documents', FileText], ['reports', 'Reports', FileText],
    ...
    <QueryState loading={active.isLoading} error={active.isError} retry={() => active.refetch()}><div className="paper-card mt-6 overflow-hidden">
      ...
      {section === 'reports' && <Reports data={reports.data} />}
    </div></QueryState>
...
function DocumentModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    event.preventDefault(); if (!file || !form.title) return; setSaving(true);
    try {
      const uploadResponse = await fetch('/api/storage/uploads/request-url', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-user-role': 'staff' }, body: JSON.stringify({ name: file.name, size: file.size, contentType: file.type || 'application/octet-stream' }) });
      if (!uploadResponse.ok) throw new Error('Upload URL unavailable');
