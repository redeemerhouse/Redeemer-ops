...
router.get("/storage/objects/*path", async (req, res) => {
  ...
  try {
    ...
    const meta = await objects.metadata(file);
    res.setHeader("Content-Type", meta.contentType ?? "application/octet-stream");
    const stream = file.createReadStream();
    const streamTimeout = setTimeout(() => {
      stream.destroy(new Error("Object storage stream timed out."));
    }, 30_000);
    streamTimeout.unref();
    const clearStreamTimeout = () => clearTimeout(streamTimeout);
    stream.once("end", clearStreamTimeout);
    stream.once("close", clearStreamTimeout);
    stream.once("error", (error) => {
      clearStreamTimeout();
      req.log.error({ errorType: error instanceof Error ? error.name : typeof error, correlationId: res.locals.correlationId }, "Document stream failed");
      if (!res.headersSent) problem(req, res, 503);
      else res.destroy();
    });
    stream.pipe(res);
  } catch (error) {
