...
6. **Release verification (H2–H4/task 14):** the expanded route inventory, contract regeneration, security tests, production configuration checks, and the database migration release check must pass as one release gate. The migration check validates the journal and runs the same non-interactive `db migrate` command used for production against a `DATABASE_URL` target. The private-pilot web and API artifacts build with their documented service settings; unrelated mockup preview configuration is not part of the publish path.

Successful startup and `200` responses demonstrate merge coherence only. The private-pilot route tests and security release gate must also pass before exposure; the runbook in `docs/private-pilot-release-checklist.md` is the operational release boundary.
